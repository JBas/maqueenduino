...
uint8_t Maqueenduino::begin(void) {
    uint8_t buf[2];
    _pWire->begin();
...
void Maqueenduino::PIDSwitch(State state) {
    uint8_t buf[1];
    buf[0] = state;
    writeReg(0x0A,buf,1);
}
...
void Maqueenduino::motorControl(Position motor, Dir direction, uint8_t speed) {
    uint8_t buf[4];
    if(speed >= 240){
        speed = 240;
    }
    ...
    switch (motor) {
        case DFROBOT_MAQUEEMPLUS_LEFT:
            buf[0] = direction;
            buf[1] = speed;
            writeReg(MAQUEENDUINO_LEFT_MOTOR_REGISTER, buf, 2);
            break;

        case DFROBOT_MAQUEEMPLUS_RIGHT:
            buf[0] = direction;
            buf[1] = speed;
            writeReg(MAQUEENDUINO_RIGHT_MOTOR_REGISTER, buf, 2);
            break;
        ...
        default:
            ...
            buf[2] = direction;
            buf[3] = speed;
            writeReg(MAQUEENDUINO_LEFT_MOTOR_REGISTER, buf, 4);
            break;
    }
}


void Maqueenduino::motorStop(Position motor) {
    motorControl(motor, CCW, 0);
}


uint8_t Maqueenduino::getSpeed(Position motor) {
    uint8_t buf[4],speed;
    readReg(0, buf, 4);
...
uint8_t Maqueenduino::getDirection(Position motor) {
    uint8_t buf[4],dir;
    readReg(0, buf, 4);
...
float Maqueenduino::getDistance(Position motor) {
    uint8_t buf[4];
    float dis;
    readReg(4, buf, 4);
...
void Maqueenduino::clearDistance(Position motor) {
...
uint8_t Maqueenduino::getLineSensor(LineSensor sensor) {
    uint8_t buf[1],state;
    readReg(0x1D, buf, 1);
...
uint16_t Maqueenduino::getGrayscale(LineSensor senser) {
...
void Maqueenduino::setLED(Position light, State state) {
    uint8_t buf[2];
    switch (light) {
        case DFROBOT_MAQUEEMPLUS_LEFT:
            buf[0] = state;
            writeReg(MAQUEENDUINO_LEFT_LED_REGISTER, buf, 1);
            break;
        case DFROBOT_MAQUEEMPLUS_RIGHT:
            buf[0] = state;
            writeReg(MAQUEENDUINO_RIGHT_LED_REGISTER, buf, 1);
            break;
        default:
            buf[0] = state;
            buf[1] = state;
            writeReg(MAQUEENDUINO_LEFT_LED_REGISTER, buf, 2);
    }
}


void Maqueenduino::servoControl(Servo servo, uint8_t angle) {
    uint8_t buf[1];
    switch (servo) {
...
uint8_t Maqueenduino::ultraSonic(Pin trig, Pin echo) {
    uint16_t d=0;
    _echoPin = echo;
    _trlgPin = trig;
...
uint64_t Maqueenduino::myPulseIn(uint32_t pin, uint32_t value, long maxDuration) {
...
uint32_t Maqueenduino::getIR(void) {
...
String Maqueenduino::getVersion(void) {
...
void Maqueenduino::writeReg(uint8_t reg, void *pBuf, size_t size) {
...
uint8_t Maqueenduino::readReg(uint8_t reg, void *pBuf, size_t size) {
    if (pBuf == NULL) {
        return 0;
    }
